...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  try {
    ...
    if (response.success) {
      toast.success(
        t("crud.successCreate", { module: t("modules.purchaseQuotation") }),
      );
      router.push("/admin/purchase-quotations");
    } else {
      toast.error(
        response.message ||
          t("crud.errorCreate", { module: t("modules.purchaseQuotation") }),
      );
    }
  } catch (error) {
  ...
  <div class="space-y-4">
    <div class="flex items-center gap-4">
      ...
      <h2 class="text-3xl font-bold tracking-tight">
        {{ $t("crud.createBtn") }} {{ $t("modules.purchaseQuotation") }}
      </h2>
    </div>
