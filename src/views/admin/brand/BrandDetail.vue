  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      if (brand.value.parentId) {
        const parentResponse = await brandService.getDetail(brand.value.parentId);
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3">
          <h2 class="text-3xl font-bold tracking-tight">{{ brand?.name || 'Brand Details' }}</h2>
          <Badge v-if="brand" :variant="brand.status ? 'success' : 'warning'" class="text-[10px] h-5 px-2 uppercase font-bold shadow-sm self-center mt-1">
