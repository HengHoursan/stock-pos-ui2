...
import { ChevronLeft, Loader2, FileText, Plus, Trash2, Truck } from "lucide-vue-next";
  ...
  try {
    ...
    if (quotationId) {
      ...
      if (qRes.success && qRes.data) {
        ...
        toast.info(t('validation.loadedFromSource', { source: t('modules.purchaseQuotation') }));
      }
    }
  } catch (error) {
...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  try {
    ...
    if (response.success) {
      toast.success(t('crud.successCreate', { module: t('modules.purchaseOrder') }));
      router.push("/admin/purchase-orders");
    } else {
      toast.error(response.message || t('crud.errorCreate', { module: t('modules.purchaseOrder') }));
    }
  } catch (error) {
...
<template>
  <div class="space-y-4">
    <div class="flex items-center gap-4">
      ...
      <h2 class="text-3xl font-bold tracking-tight">{{ $t('crud.createBtn') }} {{ $t('modules.purchaseOrder') }}</h2>
    </div>
