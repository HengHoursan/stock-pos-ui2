...
const record = ref<SaleReturn | null>(null);
const loading = ref(true);

function getStatusBadge(rec: SaleReturn) {
...
async function fetchDetail() {
  ...
  loading.value = true;
  try {
    const response = await srService.getDetail(id);
...
<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
      ...
      <div>
        <Button v-if="record && record.status === InvoiceStatus.DRAFT" @click="router.push(`/admin/sale-returns/${record.id}/edit`)" variant="outline">
          {{ crud.editBtn }}
        </Button>
      </div>
    </div>
        ...
        <Card class="shadow-sm border-muted-foreground/10 text-card-foreground">
          ...
          <CardContent class="pt-4 space-y-4 text-sm">
            <div>
              ...
              <p class="font-medium">{{ record.customer?.name || `ID: ${record.customerId}` }}</p>
            </div>
            <div>
            ...
            <div>
              <p class="text-muted-foreground mb-1">{{ fields.remarks }}</p>
            ...
            <Table>
              <TableHeader class="bg-muted/30">
                <TableRow>
                  ...
                  <TableHead class="text-right">{{ fields.quantity }}</TableHead>
              ...
              <TableBody>
                ...
                <TableRow v-else>
                  <TableCell colspan="4" class="py-12 text-center text-muted-foreground italic">
                    {{ t('common.noData') }}
                  </TableCell>
                </TableRow>
              </TableBody>
            ...
            <div class="flex justify-end p-6 border-t bg-muted/10">
              <div class="w-full max-w-sm space-y-3">
                <div class="flex justify-between items-center text-lg font-black pt-2 border-t border-primary/10">
                  <span class="text-foreground">{{ fields.totalPrice }}:</span>
