...
const { labels: productLabels } = useAppI18n("product");
  ...
  <div class="space-y-4">
    <div class="flex items-center gap-4">
      ...
      <h2 class="text-3xl font-bold tracking-tight">
        {{ t("crud.create", { module: labels.name }) }}
      </h2>
    </div>
          ...
          <CardContent class="pt-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <FormField v-slot="{ field }" name="productId">
                <FormItem>
                  <FormLabel>{{ productLabels.name }}</FormLabel>
                  <FormControl>
                    <SearchableSelect
              ...
              <FormField v-slot="{ field }" name="transactionType">
                <FormItem>
                  <FormLabel>{{ fields.transactionType }}</FormLabel>
                  <Select
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <FormField v-slot="{ componentField }" name="quantity">
                <FormItem>
                  <FormLabel>{{ fields.quantity }}</FormLabel>
                  <FormControl>
                    <Input
                      type="number"
                      step="1"
                      ...
                      @update:model-value="
                        (v) => form.setFieldValue('quantity', Number(v))
                      "
                    />
                  </FormControl>
                  <FormMessage />
              ...
              <FormField v-slot="{ field }" name="transactionDate">
                <FormItem>
                  ...
                  <FormControl>
                    <DatePicker v-bind="field" :placeholder="fields.selectDate" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              </FormField>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <FormField v-slot="{ componentField }" name="transactionCode">
                <FormItem>
                  <FormLabel>{{ fields.transactionCode }}</FormLabel>
                  <FormControl>
                    <Input
            ...
            <FormField v-slot="{ componentField }" name="remarks">
              <FormItem>
                ...
                <FormControl>
                  <Textarea :placeholder="fields.remarks" v-bind="componentField" />
                </FormControl>
                <FormMessage />
              </FormItem>
            </FormField>
        ...
        <div class="space-y-6">
          <Card class="shadow-sm border-muted-foreground/10 overflow-hidden">
            <CardHeader class="pb-3 border-b bg-muted/5 text-primary">
              <CardTitle
                class="text-sm uppercase tracking-wider flex items-center gap-2"
              >
            ...
            <CardContent class="pt-6">
              <div v-if="selectedProduct" class="space-y-4">
                <div class="flex items-center gap-4 border-b pb-4">
                  <div
                    class="h-12 w-12 rounded bg-muted flex items-center justify-center overflow-hidden border text-muted-foreground"
                  >
                    <Package v-if="!selectedProduct.photoPath" class="h-6 w-6" />
                    <img
                      v-else
                      :src="selectedProduct.photoPath"
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div
                    class="bg-muted/30 p-3 rounded-lg border border-muted-foreground/5 text-center"
                  >
                    <p
                      class="text-[10px] uppercase text-muted-foreground font-bold mb-1"
                    >
                      {{ fields.currentStock }}
                    </p>
                    <p class="text-xl font-bold">
                      {{ selectedProduct.detail?.currentStock || 0 }}
                    </p>
                  </div>
                  <div
                    class="bg-primary/5 p-3 rounded-lg border border-primary/10 text-center"
                  >
                    <p class="text-[10px] uppercase text-primary font-bold mb-1">
                      {{ fields.afterStock }}
                    </p>
                    <p class="text-xl font-bold text-primary">
                      {{
                        form.values.transactionType === TransactionType.IN
                          ? Number(selectedProduct.detail?.currentStock || 0) +
              ...
              <div
                v-else
                class="flex flex-col items-center justify-center py-10 text-muted-foreground italic text-sm text-center"
              >
                <Info class="h-8 w-8 mb-2 opacity-20" />
                <p>
                  {{ t("crud.selectParent", { module: productLabels.name }) }}
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card class="shadow-sm bg-primary/5 border-primary/20">
            <CardHeader class="pb-2">
              <CardTitle
                class="text-[10px] uppercase tracking-widest text-primary flex items-center gap-2"
              >
            ...
            <CardContent
              class="text-[11px] text-muted-foreground leading-relaxed italic"
            >
              {{ fields.internalMemoDescription }}
            </CardContent>
          </Card>
        </div>
      </div>
