...
async function fetchInvoices(customerId: number) {
  ...
  try {
    ...
    if (response.success && response.data) {
      invoices.value = response.data.data.filter((i) => !i.isCancel);
    }
  } catch (error) {
...
const formSchema = toTypedSchema(
  z.object({
    customerId: z
      .number()
      .min(1, t("validation.required", { field: customerLabels.name })),
    paymentDate: z
      .string()
  ...
  if (qInvoiceId) {
    ...
    if (res.success && res.data) {
      ...
      if (balance > 0) {
        ...
        localInputs.value[res.data.id] = formatNumberInput(
...
watch(
  ...
  (newId) => {
    if (newId) {
      ...
      if (form.values.details && form.values.details.length > 0) {
...
const availableInvoices = computed(() => {
  const selectedIds = new Set(
    form.values.details?.map((d: any) => d.saleInvoiceId),
  );
...
function addInvoice(invoiceId: any) {
  ...
  if (inv) {
    ...
    push({
      ...
      remainingItems: saleOrder
        ? saleOrder.totalLine - saleOrder.totalCloseLine
        : 0,
    });
  }
}
  ...
  <div class="space-y-4">
    ...
    <form @submit="onSubmit" class="w-full">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <Card class="lg:col-span-8 shadow-sm border-muted-foreground/10">
          ...
          <CardContent class="pt-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField v-slot="{ value, handleChange }" name="customerId">
                <FormItem class="md:col-span-2">
                  <FormLabel>{{ customerLabels.name }}</FormLabel>
                  <FormControl>
                    <SearchableSelect
                      :model-value="value"
            ...
            <div class="pt-4 border-t space-y-3">
              ...
              <div v-if="detailFields.length > 0" class="space-y-2 pt-1">
                <div
                ...
                >
                  <div class="flex-1 min-w-0">
                    <p class="font-semibold text-sm">
                      {{ (form.values.details || [])[index]?.invoiceCode }}
                    </p>
                    <p class="text-xs text-muted-foreground">
                      {{ fields.balance }}:
                      {{
                        formatCurrency(
                          (form.values.details || [])[index]?.balance || 0,
                        )
                      }}
                    </p>
                  </div>
                  ...
                  <div class="shrink-0 w-36">
                    <FormField
                      ...
                      :name="`details[${index}].paidAmount`"
                    >
                      <FormItem class="mb-0">
                        <FormControl>
                          <div class="relative">
                            <span
                              class="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-semibold text-foreground/70"
                              >{{ currencySymbol }}</span
                            >
                            <Input
                              type="text"
              ...
              <p
                v-else-if="
                  form.values.customerId && !loadingInvoices && invoices.length === 0
                "
                class="text-xs text-muted-foreground/60 text-center py-3"
              >
                {{ fields.noUnpaidInvoices }}
              </p>
            </div>
        ...
        <!-- Summary Sidebar -->
        <Card
          class="lg:col-span-4 shadow-sm border-muted-foreground/10 h-min sticky top-4"
        >
          <CardHeader class="pb-3 border-b bg-muted/5">
            <CardTitle class="text-lg flex items-center gap-2">
          ...
          <CardContent class="pt-6 space-y-6">
            <div
              class="bg-primary/5 p-6 rounded-xl border border-primary/10 flex flex-col justify-center items-center text-center"
            >
            ...
            <div class="space-y-3">
              <div class="flex justify-between text-sm">
                <span class="text-muted-foreground"
                  >{{ fields.totalInvoices }}:</span
                >
                <span class="font-bold">{{ detailFields.length }}</span>
              </div>
            </div>
          </CardContent>
